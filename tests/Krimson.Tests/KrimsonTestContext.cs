...
public class KrimsonTestContext : TestContext {
    public static readonly ILogger Log = ForContext(SourceContextPropertyName, nameof(KrimsonTestContext));
    ...
    public async Task<string> CreateTopic(string topic, int partitions) {
        var topicCreated = false;

        while (!topicCreated)
            try {
                topicCreated = await AdminClient.CreateTopic(topic, partitions, 1);
            }
            catch (Exception) {
                topicCreated = await AdminClient.CreateTopic(topic, partitions, 3);
            }

        CreatedTopics.Add(topic);

        Log.Information("topic created: {TopicName}", topic);

        return topic;
    }

    public Task<string> CreateInputTopic(string testProcessorName, int partitions) =>
        CreateTopic(GetInputTopicName(testProcessorName), partitions);

    public Task<string> CreateOutputTopic(string testProcessorName, int partitions) =>
        CreateTopic(GetOutputTopicName(testProcessorName), partitions);

    public async Task<List<RecordId>> ProduceTestMessages(string topic, int numberOfMessages = 1000) {
        var requests = Enumerable.Range(1, numberOfMessages).Select(CreateMessage).ToList();

        var operation = Log.OperationAt(LogEventLevel.Information, LogEventLevel.Error)
            .Begin("producing {NumberOfMessages} test messages", requests.Count);

        using (operation) {
            try {
                var recordIds = await MassProduceRequests(topic, requests).ToListAsync();

                operation.Complete();

                return recordIds;
            }
            catch (Exception ex) when (operation.SetExceptionAndRethrow(ex)) {
                throw; // fake throw to satisfy compiler
            }
        }

        static ProducerRequest CreateMessage(int order) {
            var id = Guid.NewGuid();
    ...
    public IAsyncEnumerable<RecordId> MassProduceRequests(string topic, List<ProducerRequest> requests, CancellationToken cancellationToken = default) {
        var channel = Channel.CreateUnbounded<RecordId>(new() {
            SingleReader = true,
            SingleWriter = true
        });
        ...
        _ = Task.Run(
            async () => {
                await using var producer = KrimsonProducer.Builder
                    .Connection(ClientConnection)
                    .UseConfluentProtobuf(SchemaRegistry)
                    .ClientId(topic)
                    .Topic(topic)
                    .Create();
                ...
                foreach (var message in requests) {
                    producer.Produce(
                        message, async result => {
                            if (result.Success) {
                                sent++;
                                await channel.Writer.WriteAsync(result.RecordId, cancellationToken);
                            }
                            else
                ...
                if(sent != requests.Count) {
                    channel.Writer.Complete(new( $"failed to produce all messages {sent}/{requests.Count}"));
                }
    ...
    public record ProcessMessagesResult(IReadOnlyCollection<KrimsonRecord> ProcessedRecords, IReadOnlyCollection<SubscriptionTopicGap> SubscriptionGap) {
        public ProcessMessagesResult AssertAllTopicsCaughtUp() {
            SubscriptionGap.All(x => x.CaughtUp).Should().BeTrue();
            return this;
        }

        public ProcessMessagesResult AssertAllMessagesProcessed(IEnumerable<RecordId> expectedRecordIds) {
            ProcessedRecords.Select(x => x.Id).Should().BeEquivalentTo(expectedRecordIds);
            return this;
        }

        public ProcessMessagesResult AssertAllMessagesProcessedByKeyAndValue(IEnumerable<KrimsonRecord> expectedRecords) {
            ProcessedRecords.Select(x => (x.Key, x.Value))
                .Should().BeEquivalentTo(expectedRecords.Select(x => (x.Key, x.Value)));
    ...
    public async Task<ProcessMessagesResult> ProcessMessages(
        Func<KrimsonProcessorBuilder, KrimsonProcessorBuilder> buildProcessor,
        int expectedNumberOfMessages, bool produceOutput = false, int timeout = 120
    ) {
        var cancellator = new CancellationTokenSource(TimeSpan.FromSeconds(timeout));
        var processed   = new List<KrimsonRecord>();
        ...
        void TestMessageHandler<T>(T msg, KrimsonProcessorContext ctx) {
            ...
            if (processed.Count == expectedNumberOfMessages)
                cancellator.CancelAfter(TimeSpan.FromSeconds(3));
        }
    ...
    public Task<ProcessMessagesResult> ProcessMessages(
        string clientId, string inputTopic, int expectedNumberOfMessages, bool produceOutput = false, int timeout = 120
    ) =>
        ProcessMessages(
            prx => prx.ClientId(clientId).InputTopic(inputTopic),
            expectedNumberOfMessages, produceOutput, timeout
        );

    public Task<ProcessMessagesResult> StreamMessages(
        Func<KrimsonProcessorBuilder, KrimsonProcessorBuilder> buildProcessor, int expectedNumberOfMessages, int timeout = 30
    ) => ProcessMessages(buildProcessor, expectedNumberOfMessages, true, timeout);

    public Task<ProcessMessagesResult> StreamMessages(
        string clientId, string inputTopic, string outputTopic, int expectedNumberOfMessages, int timeout = 30
    ) =>
        ProcessMessages(
            prx => prx
                .ClientId(clientId)
            ...
            expectedNumberOfMessages, true, timeout
        );
}