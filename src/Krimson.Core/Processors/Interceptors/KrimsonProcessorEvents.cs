...
public record ProcessorStopped(string Processor, string SubscriptionName, string[] Topics, Exception? Exception = null) : ProcessorEvent(Processor);

public record ProcessorStoppedUserHandlingError(string Processor, string SubscriptionName, string[] Topics, Exception? Exception = null) : ProcessorEvent(Processor);

/// <summary>
/// Tracked positions/offsets committed to the broker or an error while doing so.
/// </summary>
public record PositionsCommitted(string Processor, IList<TopicPartitionOffsetError> Positions, Error Error) : ProcessorEvent(Processor);
...
/// </summary>
public record InputError(string Processor, KrimsonRecord Record, IReadOnlyCollection<ProducerRequest> Output, Exception Exception) : ProcessorEvent(Processor) {
    public InputError(string processor, KrimsonRecord record, Exception exception) : this(
        processor, record, new List<ProducerRequest>(), exception
    ) { }
}
