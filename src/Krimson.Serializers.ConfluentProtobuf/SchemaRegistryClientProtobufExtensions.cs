...
public static class SchemaRegistryClientProtobufExtensions {
    static readonly ConcurrentDictionary<(int SchemaId, int MessageIndex), MessageSchema> Cache = new();
    ...
    //     var schemas = await client.GetAllSchemas(subjectPrefixes).ToListAsync().ConfigureAwait(false);
    //     
    //     foreach (var schema in schemas) 
    //         Cache[(schema.SchemaId, schema.MessageIndex)] = schema;
    // }
    ...
    public static MessageSchema GetProtobufMessageSchema(this ISchemaRegistryClient client, ReadOnlyMemory<byte> data) {
        ...
        var schemaId     = KrimsonSchemaRegistry.ParseSchemaId(data);
        var messageIndex = GetSchemaMessageIndex(data);
        
        return Cache.GetOrAdd(
            (schemaId, messageIndex),
            static (key, client) => GetMessageSchema(key.SchemaId, key.MessageIndex, client),
            client
        );
